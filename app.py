...
def get_client_ip(current_request):
    # Prioriza headers de proxy para obter o IP real do cliente.
...
def identificar_bot(user_agent_string):
    # Identifica se o User-Agent pertence a um bot. Retorna True se for um bot ou se o user_agent_string for None/vazio.
    ...
    if is_lib_bot:
        logging.info(f"Bot detectado: {user_agent_string}")
        return True
    if is_string_bot:
        logging.info(f"Bot detectado: {user_agent_string}")
        return True
...
def register_visit_in_redis():
    ...
    client_ip_for_log = get_client_ip(request)

    if not user_agent_string:
        logging.info(f"Requisição sem User-Agent (IP: {client_ip_for_log or 'N/D'}), tratando como bot.")
        return True, 200

    if identificar_bot(user_agent_string):
        logging.info(f"Acesso de bot IGNORADO. IP: {client_ip_for_log or 'N/D'}, User-Agent: {user_agent_string}")
        return True, 200
    ...
    if not client_ip:
        logging.warning("Não foi possível determinar o IP do cliente (pós-verificação de bot). Visita não rastreada.")
        return True, 200
    ...
    try:
        if redis_client.sadd(unique_ips_key_current_day, client_ip) == 1:
            ...
            logging.info(
                f"Nova visita (IP: {client_ip}, Data BRT: {date_str_for_keys} {hour_str_for_log}). Total para {date_str_for_keys}: {results[0]}")
            return True, 204
        else:
            logging.info(
                f"Visita repetida (IP: {client_ip}, Data BRT: {date_str_for_keys} {hour_str_for_log}).")
            return True, 200
    except Exception as ex:
        logging.error(f"Erro ao registrar visita (IP: {client_ip}, Data: {date_str_for_keys}): {ex}")
        return False, 500
...
def trigger_send_report():
    ...
    if CRON_SECRET and (not auth_header or auth_header != expected_token):
        logging.warning(f"Tentativa de acesso não autorizado ao /send-report. IP: {get_client_ip(request)}")
